 ...
 public:
  ...
  Set Intersection(const Set& right) const {
    ...
    for (int i = 0; i < size_; ++i) {
      for (int j = 0; j < right.size_; ++j) {
        if (values_[i] == right.values_[j])
          ++count;
      }
    }

    TheType newArray[count];
    int x = 0;

    for (int i = 0; i < size_; ++i) {
      for (int j = 0; j < right.size_; ++j) {
        if (values_[i] == right.values_[j]) {
          newArray[x] = values_[i];
          ++x;
        }
      }
    }

    newSet.SetValues(count, newArray);
  ...
  bool operator == (const Set& right) const {
    ...
    for (int i = 0; i < right.size_; ++i) {
      if (!this->IsAnElementOf(right.values_[i]))
        return false;
    }
