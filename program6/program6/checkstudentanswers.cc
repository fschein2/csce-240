// Copyright 2023 fschein (first 111 lines by bhipp)
#include<iostream>
using std::cout;
using std::endl;
#include<fstream>
...
// ADD YOUR CODE HERE
  ...
  while ( check.good() && j < number_of_questions ) {
    check >> student_answer;
    the_questions[j]->Print(true);
    cout << student_answer;
    if ( the_questions[j]->CheckAnswer(student_answer) ) {
      cout << "Student got question " << j << " correct!" << endl;
      ++count;
    } else { 
        cout << "Student got question " << j << " incorrect." << endl;
    }
    ++j;
  }
  check.close();
  ...
  cout << "Student got " << count << " number of questions correct." << endl;
